...
**Only** add if it's specifically about the Desktop application or `kodu web` view. **Do not** add for terminal, TUI, or general kodu issues.
...
**Only** add if the issue mentions "zen" or "kodu zen" or "kodu black".

If the issue doesn't have "zen" or "kodu black" in it then don't add zen label

#### core

Use for core server issues in `packages/kodu/`, excluding `packages/kodu/src/cli/cmd/tui/`.
...
TUI (`packages/kodu/src/cli/cmd/tui/...`):
...
Core (`packages/kodu/...`, excluding TUI subtree):

- thdxr for sqlite/snapshot/memory bugs and larger architectural core features
- jlongster for kodu server + API feature work (tool currently remaps jlongster -> thdxr until assignable)
