...
import type { Item, Reply, Usage } from "../../kodu/test/lib/llm-server"
import { TestLLMServer } from "../../kodu/test/lib/llm-server"
...
const seedModel = (() => {
  const [providerID = "kodu", modelID = "big-pickle"] = (
    process.env.KODU_E2E_MODEL ?? "kodu/big-pickle"
  ).split("/")
  return {
    providerID: providerID || "kodu",
...
async function promptSend(page: Page) {
  return page
    .evaluate(() => {
      ...
      const sent = win.__kodu_e2e?.prompt?.sent
      return {
  ...
  const send = async (text: string, input: { noReply: boolean; shell: boolean }) => {
    if (input.noReply) {
      ...
      const state = await page.evaluate(() => {
        const model = (window as E2EWindow).__kodu_e2e?.model?.current
...
async function seedStorage(
  ...
  await page.addInitScript(
    (args: {
      ...
      const key = "kodu.global.dat:server"
      ...
      localStorage.setItem("kodu.settings.dat:defaultServerUrl", args.serverUrl)
      ...
      localStorage.setItem("kodu.global.dat:model", JSON.stringify({ recent: [args.model], user: [], variant: {} }))
    },
