...
const koduDir = path.join(repoDir, "packages", "kodu")
...
const keepSandbox = process.env.KODU_E2E_KEEP_SANDBOX === "1"

const serverEnv = {
  ...process.env,
  KODU_DISABLE_SHARE: process.env.KODU_DISABLE_SHARE ?? "true",
  KODU_DISABLE_LSP_DOWNLOAD: "true",
  KODU_DISABLE_DEFAULT_PLUGINS: "true",
  KODU_EXPERIMENTAL_DISABLE_FILEWATCHER: "true",
  KODU_TEST_HOME: path.join(sandbox, "home"),
  XDG_DATA_HOME: path.join(sandbox, "share"),
  XDG_CACHE_HOME: path.join(sandbox, "cache"),
  XDG_CONFIG_HOME: path.join(sandbox, "config"),
  XDG_STATE_HOME: path.join(sandbox, "state"),
  KODU_E2E_PROJECT_DIR: repoDir,
  KODU_E2E_SESSION_TITLE: "E2E Session",
  KODU_E2E_MESSAGE: "Seeded for UI e2e",
  KODU_E2E_MODEL: process.env.KODU_E2E_MODEL ?? "kodu/gpt-5-nano",
  KODU_CLIENT: "app",
  KODU_STRICT_CONFIG_DEPS: "true",
...
const runnerEnv = {
  ...
  VITE_KODU_SERVER_HOST: "127.0.0.1",
  VITE_KODU_SERVER_PORT: String(serverPort),
...
try {
  ...
  if (seedExit !== 0) {
    ...
    const log = await import("../../kodu/src/util/log")
    const install = await import("../../kodu/src/installation")
    await log.Log.init({
      print: true,
    ...
    const servermod = await import("../../kodu/src/server/server")
    inst = await import("../../kodu/src/project/instance")
    server = await servermod.Server.listen({ port: serverPort, hostname: "127.0.0.1" })
    console.log(`kodu server listening on http://127.0.0.1:${serverPort}`)
