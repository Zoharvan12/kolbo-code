...
export const dict = {
  ...
  "session.new.starter.fashionCampaign.prompt": "Run a fashion campaign from the attached references (demo assets — ask me first if I want to swap in my own model, outfits, or location). Attached: @Image1 is the MODEL (full-body reference), @Image2–@Image5 are four complete OUTFIT sets, @Image6 is the ENVIRONMENT. Present a one-paragraph plan and WAIT for my approval. Then generate 4 campaign images — one per outfit: dress the woman from @Image1 in the outfit from @Image2 (then @Image3, @Image4, @Image5), keeping her identity, face and body exactly consistent, photographed in the same environment as @Image6 with the same light every time. Tag the references in each generation prompt exactly as written here. Show me the set after all four; offer to lock the model as a Visual DNA for future shoots.",
  ...
  "session.new.starter.scene.prompt": "Direct a 15-second multi-shot SCENE with Seedance 2.5. Attached: @Image1 and @Image2 are the two CHARACTERS, @Image3 is the LOCATION — demo assets, ask me first if I want to swap in my own (or my Visual DNAs). Then write the full locked-intro prompt: [GLOBAL LOOK – LOCKED] one consistent grade, [CAST – IDENTICAL IN EVERY SHOT] the two characters from @Image1 and @Image2, [LOCATION] the space from @Image3. 5-6 timecoded shots across 15s — establishing, coverage over shoulders, close-ups — with real spoken DIALOGUE between the two characters (write the actual lines, natural and terse, with emotional subtext) and synced audio. Generate as ONE video and iterate with me on motion.",
  ...
  "session.new.starter.presentation.prompt": "Create a full presentation deck. First ask me: topic, audience, the ONE thing they must remember, and roughly how many slides. Then design it as a polished HTML deck with a real narrative arc — a cold-open hook, an agenda, one idea per slide, big readable numbers for any data, generated imagery in one consistent art direction, and a strong closing slide. No walls of text: every slide headline earns its place. Show me the finished deck in the browser, then iterate slide by slide with me.",
  ...
  "session.new.starter.landing.prompt": "Build me a complete, polished landing page. First ask me: the product/idea, target audience, and one site I admire. Then design and ship a full page — hero with a strong headline and a real value proposition (no lorem ipsum anywhere), social-proof strip, three feature sections with generated imagery that matches one coherent art direction, pricing, FAQ, and a closing CTA. Dark-mode aware, responsive, real typography hierarchy. Generate the hero/section images yourself so the page ships looking finished, then open it for me to review and iterate.",
  ...
  "session.new.starter.ugc.prompt": "Create a 15-second multi-shot UGC ad VIDEO. Attached: @Image1 is the PRODUCT, @Image2 is the CREATOR — demo assets, so ask me first if I want to swap in my own. Then write the full prompt per the UGC craft: 4 connected phone shots, 15s total, 9:16, Multishot ON, restate 9:16 vertical phone frame inside every shot. NEVER write the word cinematic. The creator from @Image2 talks to camera like a friend, handles the exact product from @Image1 (label to lens at least once), phone held at arm's length with slight sway and late reframes. Available light only, deep focus, flat computational-HDR contrast. Audio: room tone and her real voice — a short spoken review, no music, no captions.",
  ...
  "session.new.starter.productAnimation.prompt": "Animate the attached product photo (@Image1 — a demo asset, ask me first if I want to swap in my own product) as a MULTI-SHOT product film: generate_elements with Seedance 2.5, 12 seconds, 480p (it's a sketch pass — cheap and fast; we upscale only a winner), the product tagged as @Image1 in every shot. Write the full locked-intro prompt (GLOBAL LOOK / CAST / LOCATION, then timecoded shots): 4 connected shots — slow hero rotation, macro material detail, dynamic light sweep, final settle — keeping the product's identity exactly as the reference in every shot.",
  "session.new.starter.music.title": "Compose a track",
  "session.new.starter.music.body": "Describe the vibe — I'll produce a full track with Suno and iterate with you.",
  "session.new.starter.music.prompt": "Compose a full music track. First ask me: the vibe/genre, the use (film soundtrack, ad, just vibes), instrumental or with vocals, and any reference artists. Then write a rich generation prompt (genre, BPM feel, instrumentation, structure, mood arc) and produce the track with Suno. Play it for me, then iterate — I may want a second contrasting take.",
  "session.new.starter.productPhotoshoot.title": "Product photoshoot",
  "session.new.starter.productPhotoshoot.body": "Studio-quality product photos, no studio — describe the look.",
  "session.new.starter.productPhotoshoot.prompt": "Run a product photoshoot from the attached product photo (@Image1 — a demo asset, ask me first if I want to swap in my own product). Present a one-paragraph shot plan and WAIT for my approval. Then produce the set — one generation per shot, keeping the product's shape, materials and label exactly as @Image1: a dramatic hero shot on a sculpted surface with hard directional light, a clean white-background pack shot, a lifestyle in-context shot, and a macro material detail. Same lighting language across the set. Show me all four, then offer angle variations of the winner.",
