...
export const dict = {
  ...
  "session.new.starter.ugc.prompt": "Create a 15-second multi-shot UGC ad VIDEO with generate_elements on Seedance 2.5 — that exact model, not Seedance 2.0 (2.0 drifts between shots; 2.5's locked intro is what keeps the cuts smooth and matched). Attached: @Image1 is the PRODUCT, @Image2 is the CREATOR — demo assets, so ask me first if I want to swap in my own. Write ONE full locked-intro prompt: [GLOBAL LOOK – LOCKED, APPLIES TO EVERY SHOT] written phone-native (never write the word cinematic), [CAST – IDENTICAL IN EVERY SHOT], [LOCATION], then timecoded shots. Header: 4 connected phone shots, 15s total, 9:16, 480p (sketch pass — never higher), Multishot ON, and restate 9:16 vertical phone frame inside every shot. Add role-mapping lines: '@Image1 defines the product's packaging, label and materials' / '@Image2 defines the creator's appearance' plus 'Do not use the image backgrounds.' The creator talks to camera like a friend, handles the exact product (label to lens at least once), phone held at arm's length with slight sway and late reframes. Available light only, deep focus, flat computational-HDR contrast. Audio: room tone and her real voice — a short spoken review, no music, no captions.",
