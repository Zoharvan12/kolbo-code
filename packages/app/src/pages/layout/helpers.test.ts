...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("kodu://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("kodu://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("kodu://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("kodu://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("kodu://open-project")).toBeUndefined()
    expect(parseDeepLink("kodu://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "kodu://open-project?directory=/a",
      "kodu://other?directory=/b",
      "kodu://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("kodu://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("kodu://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("kodu://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("kodu://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "kodu://new-session?directory=/a",
      "kodu://open-project?directory=/b",
      "kodu://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __KODU__: {
        deepLinks: ["kodu://open-project?directory=/a"],
      },
    } as unknown as Window & { __KODU__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["kodu://open-project?directory=/a"])
