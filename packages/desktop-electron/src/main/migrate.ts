...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.kodu.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `kodu.settings.dat` is special: it maps to the `kodu.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"kodu.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // kodu.settings.dat → the electron settings store ("kodu.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "kodu.global.dat").
  const storeName = filename === "kodu.settings.dat" ? "kodu.settings" : filename
