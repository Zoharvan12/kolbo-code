...
const emitDeepLinks = (urls: string[]) => {
  ...
  const pending = window.__KOLBO_CODE__.deepLinks ?? []
  window.__KOLBO_CODE__.deepLinks = [...pending, ...urls]
...
const createPlatform = (): Platform => {
  ...
  const wslHome = async () => {
    if (os !== "windows" || !window.__KOLBO_CODE__?.wsl) return undefined
    return commands.wslPath("~", "windows").catch(() => undefined)
  }

  const handleWslPicker = async <T extends string | string[]>(result: T | null): Promise<T | null> => {
    if (!result || !window.__KOLBO_CODE__?.wsl) return result
  ...
  return {
    ...
    getWslEnabled: async () => {
      ...
      return window.__KOLBO_CODE__!.wsl ?? false
    },
