...
Replace runtime Bun shell template-tag usage in `packages/kodu/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/kodu/src` is removed except:
