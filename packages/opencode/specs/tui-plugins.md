...
- The value is a semver range checked against the running Kodu version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.kodu` is absent, no check is performed (backward compatible).
...
- `kodu plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `kodu plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.kodu` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.kodu`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.kodu`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.kodu` dir or `KODU_CONFIG_DIR`, Kodu installs `@opencode-ai/plugin` into that dir and writes:
...
- Local plugins persist installed themes under the local `.kodu/themes` area near the plugin config source.
...
- `--pure` / `KODU_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.kodu/plugins/tui-smoke.tsx`
- Local vim plugin: `.kodu/plugins/tui-vim.tsx`
- Local smoke config: `.kodu/tui.json`
- Local smoke theme: `.kodu/plugins/smoke-theme.json`
