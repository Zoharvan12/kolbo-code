...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for kodu.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal kodu sessions
...
```bash
KODU_ENABLE_QUESTION_TOOL=1 kodu acp
```
...
- **Terminal Integration**: Full terminal support via kodu's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | kodu acp
```
...
ACP sessions map cleanly to kodu's internal session model:
