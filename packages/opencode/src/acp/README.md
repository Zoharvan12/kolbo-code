...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for kolbo.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal kolbo sessions
...
```bash
KOLBO_ENABLE_QUESTION_TOOL=1 kolbo acp
```
...
- **Terminal Integration**: Full terminal support via kolbo's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | kolbo acp
```
...
ACP sessions map cleanly to kolbo's internal session model:
