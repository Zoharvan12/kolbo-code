...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run kolbo with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running kolbo server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
  ...
  handler: async (args) => {
    ...
    async function share(sdk: OpencodeClient, sessionID: string) {
      ...
      if (!cfg.data) return
      if (cfg.data.share !== "auto" && !Flag.KOLBO_AUTO_SHARE && !args.share) return
    ...
    if (args.attach) {
      const headers = (() => {
        const password = args.password ?? process.env.KOLBO_SERVER_PASSWORD
        if (!password) return undefined
        const username = process.env.KOLBO_SERVER_USERNAME ?? "kolbo"
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createOpencodeClient({ baseUrl: "http://kolbo.internal", fetch: fetchFn })
      await execute(sdk)
    })
  },
})
