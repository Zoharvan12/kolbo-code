...
export function FormatError(input: unknown) {
  if (MCP.Failed.isInstance(input))
    return `MCP server "${input.data.name}" failed. Note, kolbo does not support MCP authentication yet.`
  ...
  if (Provider.ModelNotFoundError.isInstance(input)) {
    ...
    return [
      ...
      `Try: \`kolbo models\` to list available models`,
      `Or check your config (kolbo.json) provider/model names`,
    ].join("\n")
  }
