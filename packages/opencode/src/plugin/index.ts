...
import { KolboAuthPlugin } from "./kolbo"
      ...
      const state = yield* InstanceState.make<State>(
        Effect.fn("Plugin.state")(function* (ctx) {
          ...
          const client = createOpencodeClient({
            ...
            headers: Flag.KOLBO_SERVER_PASSWORD
              ? {
                  Authorization: `Basic ${Buffer.from(`${Flag.KOLBO_SERVER_USERNAME ?? "kolbo"}:${Flag.KOLBO_SERVER_PASSWORD}`).toString("base64")}`,
                }
              : undefined,
          ...
          const plugins = Flag.KOLBO_PURE ? [] : (cfg.plugin_origins ?? [])
          if (Flag.KOLBO_PURE && cfg.plugin_origins?.length) {
