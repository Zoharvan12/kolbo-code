      ...
      async function resolveSDK(model: Model, s: State) {
        try {
          ...
          options["fetch"] = async (input: any, init?: BunFetchRequestInit) => {
            ...
            // Kolbo request body rewriter. Two jobs:
            //
            // (1) Sentinel unmask — see transform.ts `kolboMediaSentinel`.
            //     AI SDK's openai-compatible serializer can't emit video_url,
            //     input_audio (URL form), or file.file_url parts, so we
            //     stage those as image_url parts with a `#kolbo-type=<mime>`
            //     fragment. Here we rewrite them into the shapes kolbo-api
            //     actually wants.
            //
            // (2) Defensive image-URL extraction. The AI SDK chain has a
            //     nasty edge case for HTTPS file URLs: `convertToModelMessages`
            //     assigns `data: part.url` as a string, then openai-compatible's
            //     `convert-to-openai-compatible-chat-messages.ts` checks
            //     `part.data instanceof URL` (false for strings) and falls
            //     into `data:${mime};base64,${convertToBase64(part.data)}`.
            //     `convertToBase64` only base64-encodes Uint8Arrays; for a
            //     string it's a no-op. Result: a malformed data URL whose
            //     "base64 content" is literally the CDN URL in plain text,
            //     e.g. "data:image/jpeg;base64,https://cdn.kolbo.ai/...".
            //     The backend's multimodal detector doesn't recognize this
            //     shape and routes the request to MiniMax text-only.
            //
            //     Fix: scan image_url parts for that pattern and replace the
            //     garbage URL with the real HTTPS CDN URL. This works
            //     regardless of whether the upstream sentinel/URL-upgrade
            //     transforms ran successfully, so it's a belt-and-braces
            //     repair that closes the loop even if the AI SDK chain
            //     changes shape in a future upgrade.
            if (model.providerID === "kolbo" && opts.body && opts.method === "POST") {
              try {
                const body = JSON.parse(opts.body as string)
                if (Array.isArray(body.messages)) {
                  ...
                  for (const msg of body.messages) {
                    ...
                    msg.content = msg.content.map((part: any) => {
                      ...
                      // Job 1: sentinel unmask for video/audio/pdf
                      if (url.includes("#kolbo-type=")) {
                        const hashIdx = url.indexOf("#kolbo-type=")
                        const realUrl = url.slice(0, hashIdx).replace(/[?&]$/, "")
                        const realMime = decodeURIComponent(url.slice(hashIdx + "#kolbo-type=".length))
                        mutated = true
                        if (realMime.startsWith("video/")) {
                          return { type: "video_url", video_url: { url: realUrl } }
                        }
                        if (realMime.startsWith("audio/")) {
                          return { type: "input_audio", input_audio: { url: realUrl } }
                        }
                        if (realMime === "application/pdf") {
                          const filename = realUrl.split("/").pop() || "document.pdf"
                          return { type: "file", file: { file_url: realUrl, filename } }
                        }
                        // Unknown sentinel mime — strip fragment, keep as image_url
                        return { type: "image_url", image_url: { url: realUrl } }
                      }

                      // Job 2: extract real HTTPS URL from AI SDK's garbage
                      // double-prefixed data URL. Pattern:
                      //   data:image/<type>;base64,https://...
                      // The "base64 content" is literally the CDN URL.
                      if (url.startsWith("data:image/")) {
                        const b64Idx = url.indexOf(";base64,")
                        if (b64Idx !== -1) {
                          const payload = url.slice(b64Idx + ";base64,".length)
                ...
                // Body wasn't JSON or had an unexpected shape — send it as-is.
                // Any real error will surface as an upstream HTTP error.
              }
            }
