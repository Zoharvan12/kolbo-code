...
const DEFAULT_TIMEOUT = Flag.KOLBO_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS || 2 * 60 * 1000
...
export const BashTool = Tool.define("bash", async () => {
  ...
  return {
    ...
    async execute(params, ctx) {
      ...
      const parsedTree = await parse(params.command, ps)
      const scan = await collect(parsedTree.rootNode, cwd, ps, shell)
      parsedTree.delete()
