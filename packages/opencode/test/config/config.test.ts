...
const managedConfigDir = process.env.KODU_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "kodu.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "kodu.json") {
...
test("ignores legacy tui keys in kodu config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://kodu.ai/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "kodu.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["KODU_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { kodu: { options: { apiKey: "{env:KODU_CONSOLE_TOKEN}" } } },
        }),
      ),
    ...
    await provideTmpdirInstance(() =>
      Config.Service.use((svc) =>
        Effect.gen(function* () {
          ...
          expect(config.provider?.["kodu"]?.options?.apiKey).toBe("st_test_token")
        }),
      ),
    ).pipe(Effect.scoped, Effect.provide(layer), Effect.runPromise)
  } finally {
    if (originalControlToken !== undefined) {
      process.env["KODU_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["KODU_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "kodu.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .kodu directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const koduDir = path.join(dir, ".kodu")
      await fs.mkdir(koduDir, { recursive: true })
      const agentDir = path.join(koduDir, "agent")
...
test("loads agents from .kodu/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const koduDir = path.join(dir, ".kodu")
      await fs.mkdir(koduDir, { recursive: true })

      const agentsDir = path.join(koduDir, "agents")
...
test("loads commands from .kodu/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const koduDir = path.join(dir, ".kodu")
      await fs.mkdir(koduDir, { recursive: true })

      const commandDir = path.join(koduDir, "command")
...
test("loads commands from .kodu/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const koduDir = path.join(dir, ".kodu")
      await fs.mkdir(koduDir, { recursive: true })

      const commandsDir = path.join(koduDir, "commands")
...
test("does not try to install dependencies in read-only KODU_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.KODU_CONFIG_DIR
  process.env.KODU_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.KODU_CONFIG_DIR
    else process.env.KODU_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable KODU_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "kodu.json"),
        JSON.stringify({ $schema: "https://kodu.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .kodu config
      const projectDir = path.join(dir, "project")
      const koduDir = path.join(projectDir, ".kodu")
      await fs.mkdir(koduDir, { recursive: true })
      ...
      // Local .kodu config with different plugins
      await Filesystem.write(
        path.join(koduDir, "kodu.json"),
        JSON.stringify({
          $schema: "https://kodu.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .kodu config with some overlapping plugins
      await Filesystem.write(
        path.join(koduDir, "kodu.json"),
        JSON.stringify({
          $schema: "https://kodu.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".kodu")
...
// Note: preload.ts sets KODU_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .kodu config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .kodu directory config enables it
      const koduDir = path.join(dir, ".kodu")
      await fs.mkdir(koduDir, { recursive: true })
      await Filesystem.write(
        path.join(koduDir, "kodu.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  globalThis.fetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/kodu")) {
      fetchedUrl = urlStr
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe("https://example.com/.well-known/kodu")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "kodu.json")
    expect(await Config.resolvePluginSpec("oh-my-kodu@2.4.3", file)).toBe("oh-my-kodu@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-kodu@2.4.3", "file:///project/.kodu/plugin/oh-my-kodu.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.kodu/plugin/demo.ts", "file:///project/.kodu/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.kodu/plugin/demo.ts"])
  })
  ...
  test("loads auto-discovered local plugins as file urls", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const koduDir = path.join(projectDir, ".kodu")
        const pluginDir = path.join(koduDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("KODU_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["KODU_DISABLE_PROJECT_CONFIG"]
    process.env["KODU_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["KODU_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["KODU_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .kodu/ directories when flag is set", async () => {
    const originalEnv = process.env["KODU_DISABLE_PROJECT_CONFIG"]
    process.env["KODU_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .kodu directory with a command
          const koduDir = path.join(dir, ".kodu", "command")
          await fs.mkdir(koduDir, { recursive: true })
          await Filesystem.write(path.join(koduDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .kodu should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["KODU_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["KODU_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["KODU_CONFIG_DIR"]
      process.env["KODU_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["KODU_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["KODU_CONFIG_DIR"]
      } else {
        process.env["KODU_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("KODU_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["KODU_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["KODU_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["KODU_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["KODU_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from KODU_CONFIG_DIR, not project
...
describe("KODU_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in KODU_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["KODU_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["KODU_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://kodu.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["KODU_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["KODU_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in KODU_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["KODU_CONFIG_CONTENT"]
...
test("parseManagedPlist strips MDM metadata keys", async () => {
  const config = await Config.parseManagedPlist(
    JSON.stringify({
      PayloadDisplayName: "Kodu Managed",
      PayloadIdentifier: "ai.kodu.managed.test",
      PayloadType: "ai.kodu.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = await Config.parseManagedPlist(
    JSON.stringify({ $schema: "https://kodu.ai/config.json" }),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://kodu.ai/config.json")
})
