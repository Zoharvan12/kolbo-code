...
test("discovers skills from .kodu/skill/ directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".kodu", "skill", "test-skill")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: test-skill
...
test("returns skill directories from Skill.dirs", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".kodu", "skill", "dir-skill")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: dir-skill
  ...
  try {
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const skillDir = path.join(tmp.path, ".kodu", "skill", "dir-skill")
...
test("discovers multiple skills from .kodu/skill/ directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir1 = path.join(dir, ".kodu", "skill", "skill-one")
      const skillDir2 = path.join(dir, ".kodu", "skill", "skill-two")
      await Bun.write(
...
test("skips skills with missing frontmatter", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".kodu", "skill", "no-frontmatter")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `# No Frontmatter
...
test("discovers global skills from ~/.claude/skills/ directory", async () => {
  ...
  const originalHome = process.env.KODU_TEST_HOME
  process.env.KODU_TEST_HOME = tmp.path
  ...
  try {
    ...
    process.env.KODU_TEST_HOME = originalHome
  }
})
...
test("properly resolves directories that skills live in", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const koduSkillDir = path.join(dir, ".kodu", "skill", "agent-skill")
      const koduSkillsDir = path.join(dir, ".kodu", "skills", "agent-skill")
      const claudeDir = path.join(dir, ".claude", "skills", "claude-skill")
      const agentDir = path.join(dir, ".agents", "skills", "agent-skill")
      await Bun.write(
        path.join(claudeDir, "SKILL.md"),
...
`,
      ...
      await Bun.write(
        path.join(koduSkillDir, "SKILL.md"),
        `---
name: opencode-skill
description: A skill in the .kodu/skill directory.
---
...
`,
      ...
      await Bun.write(
        path.join(koduSkillsDir, "SKILL.md"),
        `---
name: opencode-skill
description: A skill in the .kodu/skills directory.
---
