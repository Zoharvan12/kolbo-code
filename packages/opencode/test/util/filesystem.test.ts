...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "kodu.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "kodu.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "kodu.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "kodu.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["kodu.json", "kodu.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "kodu.json"),
        path.join(tmp.path, "kodu.jsonc"),
        path.join(project, "kodu.json"),
        path.join(project, "kodu.jsonc"),
      ])
    })
  })
