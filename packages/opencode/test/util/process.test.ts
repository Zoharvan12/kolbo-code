...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.KODU_TEST ?? "")'), {
      env: {
        KODU_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "KODU_TEST_SHELL"], {
      shell: true,
      env: {
        KODU_TEST_SHELL: "ok",
      },
    ...
    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("KODU_TEST_SHELL=ok")
  })
