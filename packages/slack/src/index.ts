...
console.log("🚀 Starting kodu server...")
const kodu = await createOpencode({
  port: 0,
})
...
;(async () => {
  const events = await kodu.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new kodu session...")
    const { client, server } = kodu
    ...
    console.log("✅ Created kodu session:", createResult.data.id)
  ...
  console.log("📝 Sending to kodu:", message.text)
