...
أنشئ الملف `.kodu/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Kodu أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Kodu:
...
- على مستوى النظام: `~/.config/kodu/commands/`
- لكل مشروع: `.kodu/commands/`
...
يتضمن kodu عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
