...
يمكنك إضافة أدوات خارجية إلى Kodu باستخدام _Model Context Protocol_، أو MCP. يدعم Kodu خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات Kodu](https://kodu.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/kodu` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى Kodu تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم Kodu بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك Kodu المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `kodu mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن Kodu الرموز بشكل آمن في `~/.local/share/kodu/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل Kodu إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات Kodu مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط Kodu بحساب Sentry الخاص بك.
