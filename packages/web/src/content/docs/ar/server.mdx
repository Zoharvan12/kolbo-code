---
title: الخادم
description: تفاعل مع خادم kodu عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `kodu serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل kodu استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `kodu.local` |
...
عيّن `KODU_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `kodu`، أو عيّن `KODU_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `kodu serve` و `kodu web`.
...
عند تشغيل `kodu` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم kodu للتفاعل مع kodu برمجيا.
:::

تتيح هذه البنية لـ kodu دعم عدة عملاء وتمكّنك من التفاعل مع kodu برمجيا.

يمكنك تشغيل `kodu serve` لبدء خادم مستقل. إذا كانت واجهة kodu في terminal (TUI)
قيد التشغيل، فسيبدأ `kodu serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Kodu لـ [IDE](/docs/ide).
...
يعرض خادم kodu واجهات API التالية.
