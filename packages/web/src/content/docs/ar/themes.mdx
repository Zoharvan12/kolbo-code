...
مع Kodu يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Kodu سمتنا `kodu`.
...
- يريدون أن يطابق Kodu مظهر terminal لديهم
...
يدعم Kodu نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/kodu/themes/*.json` أو `$XDG_CONFIG_HOME/kodu/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.kodu/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.kodu/themes/*.json`
