...
Postoje dvije vrste agenata u Kodu: primarni agenti i podagenti.
...
Kodu dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
Kodu dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

Kodu dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `kodu.json`:
...
- Globalno: `~/.config/kodu/agents/`
- Po projektu: `.kodu/agents/`
...
Ako temperatura nije navedena, Kodu koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu Kodu konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj Kodu konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [Kodu Zen](/docs/zen), koristili biste `kodu/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `kodu models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/anomalyco/kodu).
:::
