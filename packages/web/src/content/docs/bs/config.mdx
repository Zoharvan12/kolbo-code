---
title: Konfiguracija
description: Korištenje Kodu JSON konfiguracije.
---

Možete konfigurirati Kodu koristeći JSON konfiguracijski fajl.

---

## Format

Kodu podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/kodu`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/kodu/kodu.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`KODU_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`kodu.json` u projektu) - postavke specifične za projekat
5. **`.kodu` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`KODU_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.kodu` i `~/.config/kodu` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/kodu` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Kodu konfiguraciju u `~/.config/kodu/kodu.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/kodu/tui.json`.
...
Dodajte `kodu.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Kodu pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `KODU_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `KODU_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.kodu` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.kodu`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`kodu.ai/config.json`**](https://kodu.ai/config.json).

TUI konfiguracija koristi [**`kodu.ai/tui.json`**](https://kodu.ai/tui.json).
...
Koristite `KODU_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `kodu.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `kodu serve` i `kodu web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Kodu server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `kodu.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Kodu konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Kodu pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/kodu/agents/` ili `.kodu/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Kodu će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`kodu run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/kodu/commands/` ili `.kodu/commands/`. [Saznajte više ovdje](/docs/commands).
...
Kodu će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Kodu **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Kodu sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.kodu/plugins/` ili `~/.config/kodu/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Kodu da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
