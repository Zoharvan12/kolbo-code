...
Možete dodati vanjske alate u Kodu koristeći _Model Context Protocol_, ili MCP. Kodu podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Kodu Config](https://kodu.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/kodu`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Kodu automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Kodu će:
...
Ako server zahtijeva autentifikaciju, Kodu će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `kodu mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Kodu sigurno čuva tokene u `~/.local/share/kodu/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Kodu, zajedno s ugrađenim alatima. Možete njima upravljati kroz Kodu konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Kodu sa vašim Sentry nalogom.
