...
ove ili konfigurirajte svoje putem kodu config.
...
kodu dolazi sa dva ugrađena načina rada.
...
- `edit` - Ne mogu modificirati postojeće fajlove, osim fajlova koji se nalaze na `.kodu/plans/*.md` radi detaljiziranja samog plana
...
Konfigurirajte načine rada u svom `kodu.json` konfiguracijskom fajlu:
...
Ako temperatura nije navedena, kodu koristi podrazumijevane postavke specifične za model (obično 0 za većinu modela i 0.55 za Qwen modele).
...
Kreirajte fajlove načina u `.kodu/modes/` za specifične načine rada ili `~/.config/kodu/modes/` za globalne načine:
