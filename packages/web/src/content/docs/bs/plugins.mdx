---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Kodu.
---

Dodaci vam omogućavaju da proširite Kodu spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Kodu.
...
- `.kodu/plugins/` - Dodaci na nivou projekta
- `~/.config/kodu/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/kodu/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/kodu/kodu.json`)
2. Konfiguracija projekta (`kodu.json`)
3. Globalni direktorij dodataka (`~/.config/kodu/plugins/`)
4. Direktorij dodataka projekta (`.kodu/plugins/`)
...
Kodu pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Kodu.
...
Spriječite kodu da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u kodu:
...
Pomoćnik `tool` kreira prilagođeni alat koji kodu može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za kodu zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://kodu.ai/docs/sdk) za detalje.
