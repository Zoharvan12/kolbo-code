---
title: Server
description: Komunicirajte s kodu serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `kodu serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji kodu klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `kodu.local` |
...
Postavite `KODU_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `kodu`, ili postavite `KODU_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `kodu serve` i za `kodu web`.
...
Kada pokrenete `kodu`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite kodu server za programsku interakciju sa opencode-om.
:::

Ova arhitektura omogucava kodu podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `kodu serve` da startate standalone server. Ako je kodu TUI vec pokrenut, `kodu serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Kodu [IDE](/docs/ide) pluginovi.
...
kodu server izlaže sljedece API-je.
