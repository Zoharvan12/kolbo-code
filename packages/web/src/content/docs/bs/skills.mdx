...
Agent skills omogucavaju Kodu da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Kodu pretrazuje ove lokacije:

- Konfiguracija projekta: `.kodu/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/kodu/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Kodu ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.kodu/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/kodu/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.kodu/skills/git-release/SKILL.md` ovako:
...
Kodu navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `kodu.json`:
...
**Za ugradene agente** (u `kodu.json`):
