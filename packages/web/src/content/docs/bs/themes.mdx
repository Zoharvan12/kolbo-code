...
U Kodu mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Kodu koristi nasu `kodu` temu.
...
Kodu dolazi sa vise ugradenih tema.
...
- Zele da Kodu odgovara izgledu njihovog terminala
...
Kodu podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/kodu/themes/*.json` ili `$XDG_CONFIG_HOME/kodu/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.kodu/themes/*.json`
4. **Trenutni radni direktorij** - `./.kodu/themes/*.json`
