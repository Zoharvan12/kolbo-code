...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Kodu dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Kodu.
...
:::note
Ovaj alat je dostupan samo kada je `KODU_EXPERIMENTAL_LSP_TOOL=true` (ili `KODU_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Kodu provajdera ili kada je varijabla `KODU_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Kodu:
