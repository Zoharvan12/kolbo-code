...
Da biste otklonili probleme s Kodu, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\kodu\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `kodu --log-level DEBUG`.

---

## Pohrana

Kodu pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/kodu/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\kodu`
...
Kodu Desktop pokreće lokalni Kodu server (`opencode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Kodu` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/kodu/kodu.jsonc` (ili `~/.config/kodu/kodu.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/kodu/kodu.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\kodu\kodu.jsonc`
...
Kodu također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/kodu/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\kodu\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Kodu mogao ponovo izgraditi.

1. Potpuno zatvorite Kodu Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/kodu`
- **Linux**: obrišite `~/.cache/kodu` (ili pokrenite `rm -rf ~/.cache/kodu`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\kodu`

3. Ponovo pokrenite Kodu Desktop.

---

### Rješavanje problema sa vezom na serveru

Kodu Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `kodu.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `KODU_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `KODU_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Kodu Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Kodu karakteristikama.

---

### Obavještenja se ne prikazuju

Kodu Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Kodu u postavkama vašeg OS-a, i
...
1. Zatvorite Kodu Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Kodu Desktop aplikacije):

- `kodu.settings.dat` (URL zadanog servera za desktop)
- `kodu.global.dat` i `kodu.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
3. Uvjerite se da imate najnoviju verziju sa `kodu upgrade`
...
Da saznate kojim modelima imate pristup, pokrenite `kodu models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\kodu`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Kodu dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\kodu`

2. Ponovo pokrenite Kodu da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Kodu da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Kodu će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
