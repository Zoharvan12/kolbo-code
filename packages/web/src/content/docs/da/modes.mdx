...
Tilstande i kodu giver dig mulighed for at tilpasse adfærd, værktøjer og prompter til forskellige brugstilfælde.
...
kodu leveres med to indbyggede tilstande.
...
- `edit` - Kan ikke ændre eksisterende filer, undtagen filer placeret på `.kodu/plans/*.md` for at detaljere selve planen
...
Konfigurer tilstande i din `kodu.json`-konfigurationsfil:
...
Hvis der ikke er angivet nogen temperatur, bruger kodu modelspecifikke standarder (typisk 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
Opret tilstandsfiler i `.kodu/modes/` for projektspecifikke tilstande eller `~/.config/kodu/modes/` for globale tilstande:
