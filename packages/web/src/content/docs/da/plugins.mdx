---
title: Plugins
description: Skriv dine egne plugins for at udvide Kodu.
---

Plugins giver dig mulighed for at udvide Kodu ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Kodus standardadfærd.
...
- `.kodu/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/kodu/node_modules/`.
...
1. Global konfiguration (`~/.config/kodu/kodu.json`)
2. Projektkonfiguration (`kodu.json`)
3. Global plugin-mappe (`~/.config/kodu/plugins/`)
4. Projekt plugin bibliotek (`.kodu/plugins/`)
...
Kodu kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En kodu SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide kodu.
...
:::note
Hvis du bruger Kodu desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå kodu i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til kodu:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som kodu kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for kodu sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://kodu.ai/docs/sdk) for detaljer.
