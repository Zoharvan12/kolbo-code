...
Kommandoen `kodu serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en opencode-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `kodu.local` |
...
Indstil `KODU_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `kodu`, eller indstil `KODU_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `kodu serve` og `kodu web`.
...
Når du kører `kodu` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug opencode-serveren til at interagere med kodu programmatisk.
:::

Denne arkitektur lader kodu understøtte flere klienter og lader dig interagere med kodu programmatisk.

Du kan køre `kodu serve` for at starte en selvstændig server. Hvis du har
kodu TUI kørende, vil `kodu serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Kodu [IDE](/docs/ide) plugins.
...
Kodu-serveren viser følgende API'er.
