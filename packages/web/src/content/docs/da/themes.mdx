...
Med Kodu kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Kodu vårt eget `kodu`-tema.
...
Kodu kommer med flere innebygde temaer.
...
- Vil at Kodu skal matche terminalens utseende
...
Kodu støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/kodu/themes/*.json` eller `$XDG_CONFIG_HOME/kodu/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.kodu/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.kodu/themes/*.json`
