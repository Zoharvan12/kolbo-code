...
Her er alle de innebygde verktøyene som er tilgængelige i Kodu.
...
:::note
Dette verktøyet er bare tilgængelig når `KODU_EXPERIMENTAL_LSP_TOOL=true` (eller `KODU_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger Kodu-leverandøren eller når miljøvariabelen `KODU_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
