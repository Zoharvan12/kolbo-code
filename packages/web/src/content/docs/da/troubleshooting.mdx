...
For at fejlfinde problemer med Kodu, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\kodu\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `kodu --log-level DEBUG`.

---

## Lagring

kodu gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/kodu/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\kodu`
...
Kodu Desktop kører en lokal Kodu-server (`opencode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Kodu`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/kodu/kodu.jsonc` (eller `~/.config/kodu/kodu.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/kodu/kodu.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\kodu\kodu.jsonc`
...
Kodu kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/kodu/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\kodu\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Kodu kan genopbygge den.

1. Afslut Kodu Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/kodu`
- **Linux**: slet `~/.cache/kodu` (eller kør `rm -rf ~/.cache/kodu`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\kodu`

3. Start Kodu Desktop på ny.

---

### Løs problemer med serverforbindelse

Kodu Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `kodu.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `KODU_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `KODU_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Kodu Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Kodu.

---

### Meddelelser vises ikke

Kodu Desktop viser kun systemvarsler når:

- varsler er aktiveret for Kodu i dine OS-indstillinger, og
...
1. Afslut Kodu Desktop.
2. Find og slet disse filer (de findes i Kodu Desktop-appens datamappe):

- `kodu.settings.dat` (skrivebordsstandardserver URL)
- `kodu.global.dat` og `kodu.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Kodu:
...
For at finde ud af hvilke modeller du har adgang til, kør `kodu models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\kodu`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. kodu installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\kodu`

2. Start kodu på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge kodu til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
kodu vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
