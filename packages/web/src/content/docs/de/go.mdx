...
Kodu Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

:::note
Kodu Go befindet sich derzeit in der Beta-Phase.
:::

Go funktioniert wie jeder andere Provider in Kodu. Du abonnierst Kodu Go und
...
Kodu Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

Kodu Go funktioniert wie jeder andere Provider in Kodu.

1. Du meldest dich bei **<a href={console}>Kodu Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `Kodu Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann Kodu Go abonnieren.
:::
...
Kodu Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner Kodu Config verwendet das Format `opencode-go/<model-id>`. Für Kimi K2.5 würdest du beispielsweise `opencode-go/kimi-k2.5` in deiner Config verwenden.
...
Wir haben Kodu Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit Kodu zu nutzen.
