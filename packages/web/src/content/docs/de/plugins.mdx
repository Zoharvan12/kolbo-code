---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Kodu zu erweitern.
---

Mit Plugins können Sie Kodu erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Kodu zu ändern.
...
- `.kodu/plugins/` – Plugins auf Projektebene
- `~/.config/kodu/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/kodu/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/kodu/kodu.json`)
2. Projektkonfiguration (`kodu.json`)
3. Globales Plugin-Verzeichnis (`~/.config/kodu/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.kodu/plugins/`)
...
Kodu führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Kodu-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Kodu erweitern können.
...
:::note
Wenn Sie die Kodu-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Kodu `.env`-Dateien liest:
...
Plugins können Kodu auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Kodu aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Kodu zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://kodu.ai/docs/sdk).
