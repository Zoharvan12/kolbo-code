---
title: Regeln
description: Lege eigene Anweisungen fuer kodu fest.
---

Du kannst kodu ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in kodu den Befehl `/init` aus.
...
So kann kodu sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht kodu sie zu erweitern.
...
kodu kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/kodu/AGENTS.md` speichern.
...
Wenn du von Claude Code kommst, unterstuetzt Kodu dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/kodu/AGENTS.md` existiert)
...
Beim Start sucht kodu in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/kodu/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `kodu.json` oder in `~/.config/kodu/kodu.json` angeben.
...
kodu parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit kodu.json

Empfohlen ist das `instructions`-Feld in der `kodu.json`:
...
Du kannst kodu explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `kodu.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
