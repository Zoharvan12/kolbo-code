...
Agent Skills erlauben Kodu, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Kodu sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Kodu vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.kodu/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/kodu/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.kodu/skills/git-release/SKILL.md` zum Beispiel so:
...
Kodu listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `kodu.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `kodu.json`):
