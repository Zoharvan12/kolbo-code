...
Mit Kodu kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Kodu das `kodu`-Theme.
...
Kodu liefert mehrere eingebaute Themes mit.
...
- Kodu optisch ans Terminal anpassen wollen
...
Kodu unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/kodu/themes/*.json` oder `$XDG_CONFIG_HOME/kodu/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.kodu/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.kodu/themes/*.json`
