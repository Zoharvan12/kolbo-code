...
Wenn Kodu Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\kodu\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `kodu --log-level DEBUG`.

---

## Speicher

kodu speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/kodu/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\kodu`
...
Kodu Desktop startet im Hintergrund einen lokalen Kodu-Server (`opencode-cli`-Sidecar).
...
- Nur macOS: `Kodu`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/kodu/kodu.jsonc` (oder `~/.config/kodu/kodu.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/kodu/kodu.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\kodu\kodu.jsonc`
...
Kodu kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/kodu/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\kodu\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Kodu ihn neu aufbauen kann.

1. Beende Kodu Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/kodu`
- **Linux**: loesche `~/.cache/kodu` (oder fuehre aus: `rm -rf ~/.cache/kodu`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\kodu`

3. Starte Kodu Desktop neu.

---

### Server-Verbindungsprobleme beheben

Kodu Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `kodu.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `KODU_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `KODU_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Kodu Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Kodu-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Kodu Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Kodu in den OS-Einstellungen aktiviert sind, und
...
1. Beende Kodu Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Kodu Desktop):

- `kodu.settings.dat` (desktop default server URL)
- `kodu.global.dat` und `kodu.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Kodu hast:
...
3. Stelle sicher, dass du die neueste Version hast: `kodu upgrade`
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `kodu models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\kodu`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. kodu installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\kodu`

2. Starte kodu neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt kodu, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
kodu erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
