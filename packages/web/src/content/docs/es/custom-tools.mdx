...
description: Cree herramientas que LLM pueda llamar en kodu.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de kodu como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.kodu/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/kodu/tools/`.
...
```ts title=".kodu/tools/database.ts" {1}
...
```ts title=".kodu/tools/project.ts" {8}
import { tool } from "@opencode-ai/plugin"
...
```python title=".kodu/tools/add.py"
import sys
...
```ts title=".kodu/tools/python-add.ts" {10}
import { tool } from "@opencode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".kodu/tools/add.py")
