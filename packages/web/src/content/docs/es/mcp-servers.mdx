...
Puede agregar herramientas externas a Kodu utilizando el _Model Context Protocol_ o MCP. Kodu admite servidores locales y remotos.
...
Puede definir servidores MCP en su [Kodu Config](https://kodu.ai/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/kodu`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
Kodu maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, Kodu:
...
Si el servidor requiere autenticación, Kodu le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `kodu mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, Kodu almacenará los tokens de forma segura en `~/.local/share/kodu/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en Kodu, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración Kodu como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar Kodu a su cuenta Sentry.
