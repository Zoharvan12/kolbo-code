---
title: Complementos
description: Escriba sus propios complementos para extender Kodu.
---

Los complementos le permiten extender Kodu al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Kodu.
...
- `.kodu/plugins/` - Complementos a nivel de proyecto
- `~/.config/kodu/plugins/` - Complementos globales
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/kodu/node_modules/`.
...
1. Configuración global (`~/.config/kodu/kodu.json`)
2. Configuración del proyecto (`kodu.json`)
3. Directorio global de complementos (`~/.config/kodu/plugins/`)
4. Directorio de complementos del proyecto (`.kodu/plugins/`)
...
Kodu ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".kodu/plugins/my-plugin.ts"
...
```js title=".kodu/plugins/example.js"
...
- `client`: Un cliente SDK kodu para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar kodu.
...
```js title=".kodu/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "kodu"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Kodu, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que kodu lea archivos `.env`:

```javascript title=".kodu/plugins/env-protection.js"
...
```javascript title=".kodu/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a kodu:

```ts title=".kodu/plugins/custom-tools.ts"
...
El ayudante `tool` crea una herramienta personalizada a la que kodu puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para kodu junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://kodu.ai/docs/sdk) para obtener más detalles.
...
```ts title=".kodu/plugins/compaction.ts"
import type { Plugin } from "@opencode-ai/plugin"
...
```ts title=".kodu/plugins/custom-compaction.ts"
import type { Plugin } from "@opencode-ai/plugin"
