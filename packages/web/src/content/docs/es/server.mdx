---
title: Servidor
description: Interactuar con el servidor kodu a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `kodu serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente kodu puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `kodu.local` |
...
Configure `KODU_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `kodu`, o configure `KODU_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `kodu serve` como a `kodu web`.
...
Cuando ejecuta `kodu`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor kodu para interactuar con kodu mediante programación.
:::

Esta arquitectura permite que kodu admita múltiples clientes y le permite interactuar con kodu mediante programación.

Puede ejecutar `kodu serve` para iniciar un servidor independiente. Si tienes el
kodu TUI ejecutándose, `kodu serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Kodu [IDE](/docs/ide).
...
El servidor kodu expone las siguientes API.
