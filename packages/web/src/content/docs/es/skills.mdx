...
Las habilidades del agente permiten a Kodu descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Kodu busca estas ubicaciones:

- Configuración del proyecto: `.kodu/skills/<name>/SKILL.md`
- Configuración global: `~/.config/kodu/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Kodu sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.kodu/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/kodu/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.kodu/skills/git-release/SKILL.md` así:
...
Kodu enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `kodu.json`:
...
**Para agentes integrados** (en `kodu.json`):
