...
Con Kodu puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Kodu usa nuestro propio tema `kodu`.
...
Kodu viene con varios temas integrados.
...
- Quiere que Kodu coincida con la apariencia de su terminal
...
Kodu admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/kodu/themes/*.json` o `$XDG_CONFIG_HOME/kodu/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.kodu/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.kodu/themes/*.json`
