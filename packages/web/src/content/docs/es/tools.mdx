...
Las herramientas permiten que LLM realice acciones en su código base. Kodu viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Kodu.
...
:::note
Esta herramienta solo está disponible cuando `KODU_EXPERIMENTAL_LSP_TOOL=true` (o `KODU_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Kodu o cuando la variable de entorno `KODU_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Kodu:

```bash
KODU_ENABLE_EXA=1 kodu
```
