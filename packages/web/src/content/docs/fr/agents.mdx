...
Il existe deux types d'agents dans Kodu ; agents primaires et sous-agents.
...
Kodu est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
Kodu est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

Kodu est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `kodu.json` :
...
- Global : `~/.config/kodu/agents/`
- Par projet : `.kodu/agents/`

```markdown title="~/.config/kodu/agents/review.md"
---
...
Si aucune température n'est spécifiée, Kodu utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale Kodu et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration Kodu utilise le format `provider/model-id`. Par exemple, si vous utilisez [Kodu Zen](/docs/zen), vous utiliserez `kodu/gpt-5.1-codex` pour GPT 5.1 Codex.
...
:::tip
Exécutez `kodu models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/anomalyco/kodu).
:::
...
```markdown title="~/.config/kodu/agents/docs-writer.md"
---
...
```markdown title="~/.config/kodu/agents/security-auditor.md"
---
