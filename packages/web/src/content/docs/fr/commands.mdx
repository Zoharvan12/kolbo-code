...
Les commandes personnalisées s'ajoutent aux commandes intégrées telles que `/init`, `/undo`, `/redo`, `/rewind`, `/share`, `/help`. [En savoir plus](/docs/tui#commands).
...
Vous pouvez ajouter des commandes personnalisées via la configuration Kodu ou en créant des fichiers markdown dans le répertoire `commands/`.

---

### JSON

Utilisez l'option `command` dans votre Kodu [config](/docs/config) :
...
kodu comprend plusieurs commandes intégrées comme `/init`, `/undo`, `/redo`, `/rewind`, `/share`, `/help` ; [en savoir plus](/docs/tui#commands).
