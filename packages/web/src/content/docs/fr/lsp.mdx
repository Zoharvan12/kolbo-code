---
title: Serveurs LSP
description: Kodu s'intègre à vos serveurs LSP.
---

Kodu s'intègre via le protocole LSP (Language Server Protocol) pour aider le LLM à interagir avec votre base de code. Il utilise des diagnostics pour fournir des commentaires au LLM.

---

## Serveurs intégrés

Kodu est livré avec plusieurs serveurs LSP intégrés pour les langages populaires :
...
:::note
Vous pouvez désactiver les téléchargements automatiques du serveur LSP en définissant la variable d'environnement `KODU_DISABLE_LSP_DOWNLOAD` sur `true`.
:::
...
Lorsque kodu ouvre un fichier, il :
...
Vous pouvez personnaliser les serveurs LSP via la section `lsp` de votre configuration kodu.
