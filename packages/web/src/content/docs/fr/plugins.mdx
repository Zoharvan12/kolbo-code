---
title: Plugins
description: Écrivez vos propres plugins pour étendre Kodu.
---

Les plugins vous permettent d'étendre Kodu en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Kodu.
...
- `.kodu/plugins/` - Plugins au niveau du projet
- `~/.config/kodu/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/kodu/node_modules/`.
...
1. Configuration globale (`~/.config/kodu/kodu.json`)
2. Configuration du projet (`kodu.json`)
3. Répertoire global des plugins (`~/.config/kodu/plugins/`)
4. Répertoire des plugins du projet (`.kodu/plugins/`)
...
Kodu exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK kodu pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre kodu.
...
:::note
Si vous utilisez l'application de bureau Kodu, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez kodu de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à kodu :
...
L'assistant `tool` crée un outil personnalisé que kodu peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour kodu aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://kodu.ai/docs/sdk) pour plus de détails.
