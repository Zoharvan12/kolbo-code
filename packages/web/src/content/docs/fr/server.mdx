---
title: Serveur
description: Interagissez avec le serveur kodu via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `kodu serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client kodu peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `kodu.local` |
...
Définissez `KODU_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `kodu`, ou définissez `KODU_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `kodu serve` et à `kodu web`.
...
Lorsque vous exécutez `kodu`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur kodu pour interagir avec kodu par programmation.
:::

Cette architecture permet à kodu de prendre en charge plusieurs clients et vous permet d'interagir avec kodu par programme.

Vous pouvez exécuter `kodu serve` pour démarrer un serveur autonome. Si vous avez le
kodu TUI en cours d'exécution, `kodu serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Kodu [IDE](/docs/ide).
...
Le serveur kodu expose les API suivantes.
