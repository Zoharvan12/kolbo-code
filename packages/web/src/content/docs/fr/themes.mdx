...
Avec Kodu, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Kodu utilise notre propre thème `kodu`.
...
Kodu est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Kodu corresponde à l'apparence de leur terminal
...
Kodu prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/kodu/themes/*.json` ou `$XDG_CONFIG_HOME/kodu/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.kodu/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.kodu/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/kodu/themes
vim ~/.config/kodu/themes/my-theme.json
```
...
{
  "$schema": "https://kodu.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
