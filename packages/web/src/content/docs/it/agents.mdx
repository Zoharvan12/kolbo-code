...
In Kodu esistono due tipi di agenti: agenti primari e subagenti.
...
Kodu include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
Kodu include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

Kodu include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `kodu.json`:
...
- Globale: `~/.config/kodu/agents/`
- Per progetto: `.kodu/agents/`
...
Se non specifichi una temperature, Kodu usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di Kodu sia per la config specifica del progetto.
...
L'ID modello nella configurazione Kodu usa il formato `provider/model-id`. Per esempio, se stai usando [Kodu Zen](/docs/zen), useresti `kodu/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `kodu models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/anomalyco/kodu).
:::
