...
Puoi aggiungere strumenti esterni a Kodu usando il _Model Context Protocol_, o MCP. Kodu supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Kodu Config](https://kodu.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/kodu`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Kodu gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Kodu:
...
Se il server richiede autenticazione, Kodu ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `kodu mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Kodu memorizzera' i token in modo sicuro in `~/.local/share/kodu/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Kodu insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Kodu come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Kodu al tuo account Sentry.
