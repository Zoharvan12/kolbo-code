---
title: Plugin
description: Scrivi plugin per estendere Kodu.
---

I plugin ti permettono di estendere Kodu agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Kodu.
...
- `.kodu/plugins/` - plugin a livello progetto
- `~/.config/kodu/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/kodu/node_modules/`.
...
1. Config globale (`~/.config/kodu/kodu.json`)
2. Config di progetto (`kodu.json`)
3. Directory plugin globale (`~/.config/kodu/plugins/`)
4. Directory plugin di progetto (`.kodu/plugins/`)
...
Kodu esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di kodu per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere kodu.
...
:::note
Se usi l'app desktop di Kodu, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a kodu di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a kodu:
...
L'helper `tool` crea uno strumento personalizzato che kodu può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in kodu insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://kodu.ai/docs/sdk) per i dettagli.
