---
title: Server
description: Interagisci con il server kodu via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `kodu serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client kodu.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `kodu.local` |
...
Imposta `KODU_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `kodu`, oppure imposta `KODU_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `kodu serve` sia per `kodu web`.
...
Quando esegui `kodu` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server kodu per interagire con kodu in modo programmatico.
:::

Questa architettura permette a kodu di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `kodu serve` per avviare un server standalone. Se la TUI di kodu e gia in esecuzione, `kodu serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Kodu.
...
Il server kodu espone le seguenti API.
