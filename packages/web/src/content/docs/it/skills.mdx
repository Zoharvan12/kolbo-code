...
Le skill degli agenti permettono a Kodu di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Kodu cerca in queste posizioni:

- Config di progetto: `.kodu/skills/<name>/SKILL.md`
- Config globale: `~/.config/kodu/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Kodu risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.kodu/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/kodu/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.kodu/skills/git-release/SKILL.md` cosi':
...
Kodu elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `kodu.json`:
...
**Per agenti integrati** (in `kodu.json`):
