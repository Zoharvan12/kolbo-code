---
title: TUI
description: Kodu ターミナルユーザーインターフェイスの使用。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Kodu は、LLM を使用してプロジェクトを作業するための対話型ターミナルインターフェイスまたは TUI を提供します。

Kodu を実行すると、現在のディレクトリの TUI が開始されます。
...
Kodu TUI を使用する場合、「`/`」に続いてコマンド名を入力すると、アクションをすばやく実行できます。例えば：
...
Kodu にプロバイダーを追加します。利用可能なプロバイダーから選択し、その API キーを追加できます。
...
これは、サーバー/ランタイムの動作を構成する `kodu.json` とは別です。
...
カスタム TUI 設定パスをロードするには、`KODU_TUI_CONFIG` を使用します。
