...
Kodu has a list of keybinds that you can customize through `tui.json`.
...
Kodu uses a `leader` key for most keybinds. This avoids conflicts in your terminal.
...
The Kodu desktop app prompt input supports common Readline/Emacs-style shortcuts for editing text. These are built-in and currently not configurable via `kodu.json`.
...
The default keybinding for inserting a newline is **Ctrl+J** on Windows and **Shift+Enter** on macOS/Linux. All alternatives (`Ctrl+J`, `Shift+Enter`, `Ctrl+Enter`, `Alt+Enter`) work on any platform.

Some terminals don't send modifier keys with Enter by default. If you prefer using `Shift+Enter`, you may need to configure your terminal to send it as an escape sequence.
