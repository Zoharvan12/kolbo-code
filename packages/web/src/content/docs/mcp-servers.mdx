...
You can add external tools to Kodu using the _Model Context Protocol_, or MCP. Kodu supports both local and remote servers.
...
You can define MCP servers in your [Kodu Config](https://kodu.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/kodu` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Kodu automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Kodu will:
...
If the server requires authentication, Kodu will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `kodu mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Kodu will store the tokens securely in `~/.local/share/kodu/mcp-auth.json`.
...
Your MCPs are available as tools in Kodu, alongside built-in tools. So you can manage them through the Kodu config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Kodu to your Sentry account.
