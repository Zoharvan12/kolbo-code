...
:::caution
Modes are now configured through the `agent` option in the kodu config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in kodu allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the kodu config.
...
kodu comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.kodu/plans/*.md` to detail the plan itself
...
Configure modes in your `kodu.json` config file:
...
both the global kodu config and the project specific config.
...
Create mode files in `.kodu/modes/` for project-specific modes or `~/.config/kodu/modes/` for global modes:
