...
Det er to typer agenter i Kodu; primære agenter og underagenter.
...
Kodu kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Kodu kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

Kodu kommer med to innebygde primære agenter og to innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `kodu.json`:
...
- Globalt: `~/.config/kodu/agents/`
- Per prosjekt: `.kodu/agents/`
...
Hvis ingen temperatur er spesifisert, bruker Kodu modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Kodu-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i Kodu-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [Kodu Zen](/docs/zen), vil du bruke `kodu/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `kodu models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/anomalyco/kodu).
:::
