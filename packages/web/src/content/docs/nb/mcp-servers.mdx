...
Du kan legge til eksterne verktøy i Kodu ved å bruke _Model Context Protocol_ eller MCP. Kodu støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Kodu Config](https://kodu.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="kodu.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/kodu`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Kodu håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Kodu:
...
Hvis serveren krever autentisering, vil Kodu be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `kodu mcp auth <server-name>`.
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Kodu lagre tokens sikkert i `~/.local/share/kodu/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i Kodu, sammen med innebygde verktøy. Så du kan administrere dem gjennom Kodu-konfigurasjonen som alle andre verktøy.
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Kodu til Sentry-kontoen din.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Kodu, kan du alltid konfigurere den selv.
...
```jsonc title="~/.config/kodu/kodu.jsonc"
{
  "$schema": "https://kodu.ai/config.json",
  "provider": {
    "helicone": {
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk å logge hver Kodu-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Kodu-samtale oppført som en separat økt.
