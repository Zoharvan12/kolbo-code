...
:::caution
Moduser konfigureres nå gjennom alternativet `agent` i Kodu-konfigurasjonen.
Alternativet `mode` er nå utdatert. [Finn ut mer](/docs/agents).
:::

Moduser i Kodu lar deg tilpasse oppførselen, verktøyene og prompter for ulike brukstilfeller.

Den kommer med to innebygde moduser: **bygg** og **plan**. Du kan tilpasse
disse eller konfigurer din egen gjennom Kodu-konfigurasjonen.
...
Kodu kommer med to innebygde moduser.
...
- `edit` - Kan ikke endre eksisterende filer, bortsett fra filer som ligger på `.kodu/plans/*.md` for å detaljere selve planen
...
Konfigurer moduser i din `kodu.json` konfigurasjonsfil:
...
Hvis ingen temperatur er spesifisert, bruker Kodu modellspesifikke standardinnstillinger (vanligvis 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
Denne banen er relativ til der konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Kodu-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Opprett modusfiler i `.kodu/modes/` for prosjektspesifikke moduser eller `~/.config/kodu/modes/` for globale moduser:
