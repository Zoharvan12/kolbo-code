---
title: Tjener
description: Samhandle med Kodu-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `kodu serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Kodu-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `kodu.local` |
...
Sett `KODU_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `kodu` som standard, eller sett `KODU_SERVER_USERNAME` for å overstyre det. Dette gjelder både `kodu serve` og `kodu web`.
...
Når du kjører `kodu` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Kodu-serveren til å samhandle med Kodu programmatisk.
:::

Denne arkitekturen lar Kodu støtte flere klienter og lar deg samhandle med Kodu programmatisk.

Du kan kjøre `kodu serve` for å starte en frittstående server. Hvis du har
Kodu TUI kjører, vil `kodu serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Kodu [IDE](/docs/ide) plugins.
