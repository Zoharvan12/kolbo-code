...
Agentferdigheter lar Kodu oppdage gjenbrukbare instruksjoner fra repo- eller hjemmekatalogen din.
...
kodu søker etter disse stedene:

- Prosjektkonfigurasjon: `.kodu/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/kodu/skills/<name>/SKILL.md`
...
For prosjektlokale stier går kodu opp fra din nåværende arbeidskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.kodu/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/kodu/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.kodu/skills/git-release/SKILL.md` slik:
...
kodu viser tilgjengelige ferdigheter i `skill`-verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved å bruke mønsterbaserte tillatelser i `kodu.json`:
...
**For innebygde agenter** (i `kodu.json`):
