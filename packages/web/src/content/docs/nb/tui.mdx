...
Kodu gir et interaktivt terminalgrensesnitt (TUI) for å jobbe med prosjektene dine med en LLM.

Å kjøre Kodu starter TUI for gjeldende katalog.
...
Når du bruker Kodu TUI, kan du skrive `/` etterfulgt av et kommandonavn for raskt å utføre handlinger. For eksempel:
...
Legg til en leverandør til Kodu. Lar deg velge fra tilgjengelige leverandører og legge til deres API-nøkler.
...
Dette er atskilt fra `kodu.json`, som konfigurerer server-/kjøretidsoppførsel.
...
Bruk `KODU_TUI_CONFIG` for å laste en egendefinert TUI-konfigurasjonsbane.
