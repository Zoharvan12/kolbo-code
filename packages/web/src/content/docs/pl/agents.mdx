...
W Kodu obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
Kodu zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
Kodu ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

Kodu ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `kodu.json`:
...
- Globalnie: `~/.config/kodu/agents/`
- W projekcie: `.kodu/agents/`
...
Jeśli nie zostanie skonfigurowana, Kodu używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji Kodu, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji Kodu w `provider/model-id`. Na przykład, [Kodu Zen](/docs/zen), użyj `kodu/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `kodu models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/anomalyco/kodu).
:::
