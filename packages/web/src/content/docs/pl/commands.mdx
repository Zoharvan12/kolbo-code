...
Polecenia są dodatkiem do wbudowanych, takich jak `/init`, `/undo`, `/redo`, `/rewind`, `/share`, `/help`. [Dowiedz się więcej](/docs/tui#commands).
...
Możesz zdefiniować polecenia w `kodu.json` lub w plikach w katalogu `commands/`.

---

### JSON

Użyj opcji `command` w swojej konfiguracji kodu:
...
- Globalnie: `~/.config/kodu/commands/`
- Na projekt: `.kodu/commands/`
...
kodu zawiera kilka wbudowanych poleceń, takich jak `/init`, `/undo`, `/redo`, `/rewind`, `/share`, `/help`; [dowiedz się więcej](/docs/tui#commands).
