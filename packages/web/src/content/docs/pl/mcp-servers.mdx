...
Możesz dodać dodatkowe informacje do kodu za pomocą _Model Context Protocol_, w skrócie MCP. kodu obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [kodu Config](https://kodu.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/kodu`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
kodu automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, kodu:
...
Jeśli serwer wymaga uwierzytelnienia, kodu poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `kodu mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji kodu bezpieczne przechowa tokeny w `~/.local/share/kodu/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w kodu, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie kodu, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć kodu z kontem Sentry.
