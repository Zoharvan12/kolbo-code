...
:::caution
Tryby są teraz konfigurowane za pomocą opcji `agent` w konfiguracji kodu.
...
te lub skonfiguruj własne za pomocą konfiguracji kodu.
...
- `edit` — Nie można zastosować naruszenia plików, z naruszeniem praw autorskich w `.kodu/plans/*.md` w celu uszczegółowienia samego planu
...
Skonfiguruj tryb w pliku konfiguracyjnym `kodu.json`:
...
Jeśli nie ma wpływu na temperaturę, kodu stosuje się narzędzia badawcze dla modelu (zwykle 0 dla największych modeli, 0,55 dla modeli Qwen).
...
zarówno globalna kodu, jak i specjalna dla projektu.
...
Utwórz pliki trybów w `.kodu/modes/` dla trybów zapisanych dla projektu lub `~/.config/kodu/modes/` dla trybów globalnych:
