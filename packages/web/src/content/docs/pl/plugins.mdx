---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć kodu.
---

Wtyczki umożliwiające wyłączenie kodu poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania kodu.
...
- `.kodu/plugins/` - Wtyczki na poziomie projektu
- `~/.config/kodu/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/kodu/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/kodu/kodu.json`)
2. Konfiguracja projektu (`kodu.json`)
3. Globalny katalog wtyczek (`~/.config/kodu/plugins/`)
4. Katalog wtyczek projektu (`.kodu/plugins/`)
...
kodu uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z kodu do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do kodu.
...
:::note
Jeśli korzystasz z aplikacji komputerowej kodu, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj kodu czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do kodu:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod kodu. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://kodu.ai/docs/sdk).
