---
title: Serwer
description: Interakcja z serwerem kodu za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `kodu serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient kodu.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `kodu.local` |
...
Ustaw `KODU_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `kodu` lub ustaw `KODU_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `kodu serve`, jak i `kodu web`.
...
Kiedy uruchomisz `kodu`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera kodu do programowej interakcji z kodu.
:::

Ta architektura umożliwia obsługę wielu klientów przez kodu i programową interakcję z kodem otwartym.

Możesz uruchomić `kodu serve`, aby uruchomić samodzielny serwer. Jeśli masz
kodu TUI działa, `kodu serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki kodu [IDE](/docs/ide).
...
Serwer kodu udostępnia następujące interfejsy API.
