...
Umiejętności agenta pozwalają kodu odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
kodu przeszukuje te lokalizacje:

- Project config: `.kodu/skills/<name>/SKILL.md`
- Global config: `~/.config/kodu/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu kodu przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.kodu/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/kodu/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.kodu/skills/git-release/SKILL.md` w ten sposób:
...
kodu wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `kodu.json`:
...
**Dla agentów wbudowanych** (w `kodu.json`):
