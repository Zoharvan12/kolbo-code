...
Dzięki kodu możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie kodu używa naszego własnego motywu `kodu`.
...
kodu ma kilka wbudowanych motywów.
...
- Chcą, aby Kodu pasował do wyglądu ich terminala
...
kodu obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/kodu/themes/*.json` lub `$XDG_CONFIG_HOME/kodu/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.kodu/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.kodu/themes/*.json`
