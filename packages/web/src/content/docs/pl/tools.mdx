...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Kodu posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Kodu.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `KODU_EXPERIMENTAL_LSP_TOOL=true` (lub `KODU_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Kodu lub gdy zmienna środowiskowa `KODU_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Kodu:
