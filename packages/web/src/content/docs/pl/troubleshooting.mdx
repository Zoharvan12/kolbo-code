...
Aby debugować problemy z kodu, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\kodu\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `kodu --log-level DEBUG`.

---

## Przechowywanie danych

kodu przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/kodu/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\kodu`
...
kodu Desktop uruchamia w tle lokalny serwer kodu (sidecar `opencode-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `Kodu` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/kodu/kodu.jsonc` (lub `~/.config/kodu/kodu.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/kodu/kodu.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\kodu\kodu.jsonc`
...
kodu może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/kodu/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\kodu\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby kodu mógł ją odbudować.

1. Całkowicie zamknij kodu Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/kodu`
- **Linux**: usuń `~/.cache/kodu` (lub uruchom `rm -rf ~/.cache/kodu`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\kodu`

3. Uruchom ponownie kodu Desktop.

---

### Napraw problemy z połączeniem z serwerem

kodu Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `kodu.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `KODU_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `KODU_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows kodu Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami kodu.

---

### Brak powiadomień

kodu Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla kodu w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie kodu Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji kodu Desktop):

- `kodu.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `kodu.global.dat` i `kodu.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
3. Upewnij się, że masz najnowszą wersję z `kodu upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `kodu models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\kodu`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. kodu dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\kodu`

2. Uruchom ponownie kod kodu, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to kodu do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
kodu wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
