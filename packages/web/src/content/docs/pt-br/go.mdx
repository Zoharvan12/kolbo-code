...
O Kodu Go é uma assinatura de baixo custo — **US$ 5 no seu primeiro mês**, depois **US$ 10/mês** — que oferece acesso confiável a modelos abertos de programação populares.

:::note
O Kodu Go está atualmente em beta.
:::

O Go funciona como qualquer outro provedor no Kodu. Você assina o Kodu Go e
...
O Kodu Go lhe dá acesso a esses modelos por **US$ 5 no seu primeiro mês**, depois **US$ 10/mês**.

---

## Como funciona

O Kodu Go funciona como qualquer outro provedor no Kodu.

1. Você entra no **<a href={console}>Kodu Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `Kodu Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o Kodu Go.
:::
...
O Kodu Go inclui os seguintes limites:
...
O [ID do modelo](/docs/config/#models) na sua configuração do Kodu
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o Kodu também.
