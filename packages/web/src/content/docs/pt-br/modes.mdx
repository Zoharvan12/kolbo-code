...
:::caution
Os modos agora são configurados através da opção `agent` na configuração do kodu. A opção `mode` agora está obsoleta. [Saiba mais](/docs/agents).
:::

Os modos no kodu permitem que você personalize o comportamento, as ferramentas e os prompts para diferentes casos de uso.

Ele vem com dois modos integrados: **build** e **plan**. Você pode personalizar esses ou configurar os seus próprios através da configuração do kodu.
...
O kodu vem com dois modos integrados.
...
- `edit` - Não pode modificar arquivos existentes, exceto para arquivos localizados em `.kodu/plans/*.md` para detalhar o plano em si
...
Configure os modos no seu arquivo de configuração `kodu.json`:
...
- Global: `~/.config/kodu/modes/`
- Projeto: `.kodu/modes/`

```markdown title="~/.config/kodu/modes/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
Se nenhuma temperatura for especificada, o kodu usa padrões específicos do modelo (geralmente 0 para a maioria dos modelos, 0.55 para modelos Qwen).
...
Este caminho é relativo a onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do kodu quanto para a configuração específica do projeto.
...
Crie arquivos de modo em `.kodu/modes/` para modos específicos do projeto ou `~/.config/kodu/modes/` para modos globais:

```markdown title=".kodu/modes/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/kodu/modes/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
