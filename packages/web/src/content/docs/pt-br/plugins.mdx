---
title: Plugins
description: Escreva seus próprios plugins para estender o kodu.
---

Plugins permitem que você estenda o kodu conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do kodu.
...
- `.kodu/plugins/` - Plugins em nível de projeto
- `~/.config/kodu/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/kodu/node_modules/`.
...
1. Configuração global (`~/.config/kodu/kodu.json`)
2. Configuração do projeto (`kodu.json`)
3. Diretório de plugins global (`~/.config/kodu/plugins/`)
4. Diretório de plugins do projeto (`.kodu/plugins/`)
...
O kodu executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do kodu para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o kodu.
...
:::note
Se você estiver usando o aplicativo desktop kodu, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o kodu de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao kodu:
...
O helper `tool` cria uma ferramenta personalizada que o kodu pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o kodu junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://kodu.ai/docs/sdk) para detalhes.
