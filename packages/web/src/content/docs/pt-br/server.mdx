---
title: Servidor
description: Interaja com o servidor kodu via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `kodu serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente kodu pode usar.
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `kodu.local` |
...
Defina `KODU_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `kodu`, ou defina `KODU_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `kodu serve` quanto ao `kodu web`.

```bash
KODU_SERVER_PASSWORD=your-password kodu serve
```
...
Quando você executa `kodu`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor kodu para interagir com o kodu programaticamente.
:::

Essa arquitetura permite que o kodu suporte múltiplos clientes e permite que você interaja com o kodu programaticamente.

Você pode executar `kodu serve` para iniciar um servidor autônomo. Se você tiver o TUI do kodu em execução, `kodu serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do kodu [IDE](/docs/ide).
...
O servidor kodu expõe as seguintes APIs.
