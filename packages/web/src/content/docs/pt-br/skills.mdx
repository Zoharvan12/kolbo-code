...
As habilidades do agente permitem que o kodu descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O kodu pesquisa nesses locais:

- Configuração do projeto: `.kodu/skills/<name>/SKILL.md`
- Configuração global: `~/.config/kodu/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o kodu sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.kodu/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/kodu/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.kodu/skills/git-release/SKILL.md` assim:
...
O kodu lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `kodu.json`:
...
**Para agentes embutidos** (em `kodu.json`):
