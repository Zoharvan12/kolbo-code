...
Com o kodu, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o kodu usa nosso próprio tema `kodu`.
...
O kodu vem com vários temas embutidos.
...
- Querem que o kodu corresponda à aparência do seu terminal
...
O kodu suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/kodu/themes/*.json` ou `$XDG_CONFIG_HOME/kodu/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.kodu/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.kodu/themes/*.json`
