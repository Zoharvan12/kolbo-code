...
Пользовательские команды дополняют встроенные команды, такие как `/init`, `/undo`, `/redo`, `/rewind`, `/share`, `/help`. [Подробнее](/docs/tui#commands).
...
Создайте `.kodu/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию kodu или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем kodu [config](/docs/config):
...
- Глобальный: `~/.config/kodu/commands/`
- Для каждого проекта: `.kodu/commands/`
...
kodu включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/rewind`, `/share`, `/help`; [подробнее](/docs/tui#commands).
