---
title: Server
description: Interact with kodu server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `kodu serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an kodu client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `kodu.local` |
...
Set `KODU_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `kodu`, or set `KODU_SERVER_USERNAME` to override it. This applies to both `kodu serve` and `kodu web`.
...
When you run `kodu` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the kodu server to interact with kodu programmatically.
:::

This architecture lets kodu support multiple clients and allows you to interact with kodu programmatically.

You can run `kodu serve` to start a standalone server. If you have the
kodu TUI running, `kodu serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Kodu [IDE](/docs/ide) plugins.
...
The kodu server exposes the following APIs.
