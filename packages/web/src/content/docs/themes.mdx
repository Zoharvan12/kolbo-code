...
With Kodu you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Kodu uses our own `kodu` theme.
...
Kodu comes with several built-in themes.
...
- Want Kodu to match their terminal's appearance
...
Kodu supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/kodu/themes/*.json` or `$XDG_CONFIG_HOME/kodu/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.kodu/themes/*.json`
4. **Current working directory** - Defined in `./.kodu/themes/*.json`
