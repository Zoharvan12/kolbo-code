...
kodu'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
kodu, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
kodu, **General** ve **Explore** olmak üzere iki yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

kodu iki yerleşik birincil agent ve iki yerleşik alt agent ile birlikte gelir.
...
Agent'ları `kodu.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse kodu modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global kodu yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
kodu yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [Kodu Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `kodu/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `kodu models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/anomalyco/kodu).
:::
