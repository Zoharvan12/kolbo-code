...
Özel komutlar, `/init`, `/undo`, `/redo`, `/rewind`, `/share`, `/help` gibi komutlara ek olarak sunulur. [Daha fazla bilgi](/docs/tui#commands).
...
kodu işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

kodu [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/kodu/commands/`
- Proje başına: `.kodu/commands/`
...
kodu, `/init`, `/undo`, `/redo`, `/rewind`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
