...
description: Kodu, LSP sunucularınızla bütünleşir.
---

Kodu, LLM'in kod tabanınızla etkileşime girmesine yardımcı olmak için Dil Sunucusu Protokolünüzle (LSP) bütünleşir. LLM'ye geri bildirim sağlamak için tanılamayı kullanır.

---

## Yerleşik

Kodu, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`KODU_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP sunucularını, kodu yapılandırmanızdaki `lsp` bölümü aracılığıyla özelleştirebilirsiniz.
