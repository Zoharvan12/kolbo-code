...
_Model Bağlam Protokolü_ veya MCP kullanarak kodu'a harici araçlar ekleyebilirsiniz. kodu hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [kodu Config](https://kodu.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/kodu` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
kodu, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde kodu şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa kodu, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `kodu mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından kodu, belirteçleri `~/.local/share/kodu/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra kodu'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi kodu yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve kodu'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
