...
:::caution
Modlar artık kodu yapılandırmasındaki `agent` seçeneği aracılığıyla yapılandırılıyor.
`mode` seçeneği artık kullanımdan kaldırıldı. [Daha fazla bilgi](/docs/agents).
:::

kodu'daki modlar, farklı kullanım durumları için davranışı, araçları ve istemleri özelleştirmenize olanak tanır.

İki yerleşik modla birlikte gelir: **build** ve **plan**. Kişiselleştirebilirsiniz
bunları veya kodu yapılandırması aracılığıyla kendinizinkini yapılandırın.
...
kodu iki yerleşik modla birlikte gelir.
...
- `edit` - Planın kendisini detaylandırmak için `.kodu/plans/*.md` adresinde bulunan dosyalar dışında mevcut dosyalar değiştirilemez
...
`kodu.json` yapılandırma dosyanızdaki modları yapılandırın:
...
Sıcaklık belirtilmezse, kodu modele özgü varsayılanları kullanır (çoğu model için genellikle 0, Qwen modelleri için 0,55).
...
hem global kodu yapılandırması hem de projeye özel yapılandırma.
...
Projeye özel modlar için `.kodu/modes/` veya genel modlar için `~/.config/kodu/modes/`'de mod dosyaları oluşturun:
