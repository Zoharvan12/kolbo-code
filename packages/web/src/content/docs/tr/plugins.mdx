---
title: Eklentiler
description: Kodu'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Kodu'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Kodu'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.kodu/plugins/` - Proje düzeyinde eklentiler
- `~/.config/kodu/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/kodu/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/kodu/kodu.json`)
2. Project config (`kodu.json`)
3. Global eklenti dizini (`~/.config/kodu/plugins/`)
4. Proje eklenti dizini (`.kodu/plugins/`)
...
Kodu bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Kodu SDK istemcisi.
...
Kodu'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Kodu masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Kodu'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Kodu'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Kodu'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Kodu'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://kodu.ai/docs/sdk) bakın.
