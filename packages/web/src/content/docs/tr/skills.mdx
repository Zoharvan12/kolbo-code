...
Ajan becerileri, kodu'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
kodu su konumlari tarar:

- Proje konfigurasyonu: `.kodu/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/kodu/skills/<name>/SKILL.md`
...
Proje yerel yollarinda kodu, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.kodu/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/kodu/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.kodu/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
kodu, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`kodu.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`kodu.json` icinde):
