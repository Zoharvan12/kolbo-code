...
kodu ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak kodu kendi `kodu` temasını kullanır.
...
kodu birden fazla yerleşik temayla gelir.
...
- kodu'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
kodu, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/kodu/themes/*.json` veya `$XDG_CONFIG_HOME/kodu/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.kodu/themes/*.json`
4. **Geçerli çalışma dizini** - `./.kodu/themes/*.json`
