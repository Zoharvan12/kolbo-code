---
title: TUI
description: Kodu terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Kodu, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

Kodu'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
Kodu TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
Kodu'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
Kodu'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `kodu.json` dosyasından ayrıdır.
...
Özel bir TUI yapılandırma yolu yüklemek için `KODU_TUI_CONFIG` kullanın.
