---
title: TUI
description: Using the Kodu terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Kodu provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Kodu starts the TUI for the current directory.
...
When using the Kodu TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Kodu. Allows you to select from available providers and add their API keys.
...
This is separate from `kodu.json`, which configures server/runtime behavior.
...
Use `KODU_TUI_CONFIG` to load a custom TUI config path.
