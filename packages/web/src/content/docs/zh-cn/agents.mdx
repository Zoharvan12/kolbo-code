...
Kodu 内置了两个主代理：**Build** 和 **Plan**。我们将在下面介绍它们。
...
Kodu 内置了两个子代理：**General** 和 **Explore**。我们将在下面介绍它们。
...
如果未指定温度，Kodu 将使用模型特定的默认值；大多数模型通常为 0，Qwen 模型为 0.55。
...
此路径相对于配置文件所在位置。因此它同时适用于全局 Kodu 配置和项目级配置。
...
Kodu 配置中的模型 ID 使用 `provider/model-id` 格式。例如，如果您使用 [Kodu Zen](/docs/zen)，则可以使用 `kodu/gpt-5.1-codex` 来表示 GPT 5.1 Codex。
...
:::tip
您有想要分享的代理吗？[提交 PR](https://github.com/anomalyco/kodu)。
:::
